...
import util
from config import DDDMEVCConfig
...
class DDDMEVC(nn.Module):
    """Decoupled Denoising Diffusion model for emotional voice conversion"""

    def __init__(self, cfg: DDDMEVCConfig, sample_rate: int) -> None:
        super().__init__()
        self.speaker_encoder = MetaStyleSpeech(cfg.style_encoder.speaker_encoder)
    ...
    def load_pretrained(
        ...
        model_mapping = {
            "speaker_encoder": (self.speaker_encoder, "metastylespeech.pth"),
    ...
    def _cond_encode(
        ...
        spk = self.speaker_encoder(x_mel, x_mask)
    ...
    def voice_conversion(
        ...
        g = self._cond_encode(y, y_mel, y_mask).unsqueeze(-1)  # (B, C, 1)
    ...
    def forward(
        ...
        x_mask = util.sequence_mask(x_n_frames, x_mel.size(2)).to(x_mel.dtype)
        g = self._cond_encode(x, x_mel, x_mask).unsqueeze(-1)  # (B, C, 1)
        return self._forward(x, x_mel, x_mask, g, n_time_steps, return_enc_out)
